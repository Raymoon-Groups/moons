...
export function CoverPhotoBanner({
  ...
  function handleFilePick(file: File) {
    ...
    if (file.size > 12 * 1024 * 1024) {
      setError('Cover photo must be 12 MB or smaller');
      return;
    }
    setError('');
  ...
  async function handleCropComplete(file: File) {
    if (file.size > 5 * 1024 * 1024) {
      setError('Cropped cover photo must be 5 MB or smaller');
      handleCropCancel();
      return;
    }
    if (cropSrc) URL.revokeObjectURL(cropSrc);
  ...
  return (
    <>
      <ImageCropModal
        open={!!cropSrc}
        imageSrc={cropSrc}
        fileName={cropMeta?.name ?? 'banner.jpg'}
        aspect="cover"
        onCancel={handleCropCancel}
        onComplete={(file) => void handleCropComplete(file)}
      />
