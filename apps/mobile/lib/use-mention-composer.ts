...
export function invalidateMentionPeopleCache() {
...
export function useMentionComposer(initialMentions: MentionRef[] = []) {
  ...
  const active = useMemo(() => getActiveMention('', 0), []);

  const getActive = useCallback((text: string, nextCaret = caret) => getActiveMention(text, nextCaret), [caret]);
