/** Feed post media limits — keep API posts controller/service + web/mobile composers in sync. */
...
export const POST_MEDIA_ACCEPT =
  'image/jpeg,image/png,image/webp,image/gif,image/heic,image/heif,video/mp4,video/webm,video/quicktime';

export function isPostImageMime(mime: string) {
  return (POST_IMAGE_MIME_TYPES as readonly string[]).includes(mime.toLowerCase());
}

export function isPostVideoMime(mime: string) {
  return (POST_VIDEO_MIME_TYPES as readonly string[]).includes(mime.toLowerCase());
}
